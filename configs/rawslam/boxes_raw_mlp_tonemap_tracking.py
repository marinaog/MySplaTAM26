...
config = dict(
    ...
    checkpoint_time_idx=800,
    save_checkpoints=True, # Save Checkpoints
    checkpoint_interval=400, # Checkpoint Interval
    use_wandb=True,
    wandb=dict(
        project="SplaTAM",
        name=run_name,
    ...
    data=dict(
        ...
        gradslam_data_cfg=f"./configs/data/rawslam/{scene_name}.yaml",
    ...
    tracking=dict(
        ...
        lrs=dict(
            ...
            logit_opacities=0.0,
            log_scales=0.0,
            # MLP params frozen during tracking — only camera pose is optimised
            features_dc=0.0,
            features_rest=0.0,
            color_mlp=0.0,
    ...
    mapping=dict(
        ...
        lrs=dict(
            means3D=0.0001,
            rgb_colors=0.0,       # dead param when use_mlp=True; kept at 0 to avoid wasted updates
